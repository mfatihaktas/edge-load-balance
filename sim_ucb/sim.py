...
def sim_ET_single_run():
	...
	ET, EW = sim_ucb(num_req_to_finish=num_req_to_finish, num_sim=1, write_to_json=True)
...
def sim_ET_vs_ro():
	...
	for ro in [0.2, 0.5, 0.65, 0.8, 0.9]:
		...
		ET, EW = sim_ucb(num_req_to_finish=num_req_to_finish, ro=ro, w=w, num_sim=num_sim, write_to_json=True)
		log(INFO, "", ET=ET, EW=EW)
		ET_l.append(ET)
		EW_l.append(EW)

	write_to_file(data=json.dumps(list(zip(ro_l, ET_l))), fname=get_filename_json(header='ro_ET_l_ucb_w_{}'.format(w), ro=ro))
	write_to_file(data=json.dumps(list(zip(ro_l, EW_l))), fname=get_filename_json(header='ro_EW_l_ucb_w_{}'.format(w), ro=ro))
